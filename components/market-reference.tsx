...
/** Just inside the sixty-second observation lifetime, so a displayed reading stays current. */
const PANEL_REFRESH_MS = 55_000;
const EQUITY_INTRO = 'Compare your planning estimate with the underlying equity reference using verified scaled units and a separate USDC/USD price. Your later purchase order still needs its own review.';
/** Describes only what this panel is showing, so it never advertises a comparison the response did not carry. */
const SCOPE_INTRO = 'This panel shows the verified Pyth readings behind your plan and states exactly what each one covers. Your later purchase order still needs its own review.';
...
function ObservationCard({ observation, label, now }: { observation?: Observation; label: string; now: number }) {
  ...
  return <div className={styles.observation}>
    ...
    </> : <p className={styles.detail}>This card shows a price only from its own verified Pyth reading.</p>}
  </div>;
}
...
export function MarketReference({ assets, quotes, mode, planIdentity, now, enabled, onData }: Props) {
  ...
  // The plan and quote a reading belongs to. A re-read of the same plan replaces its
  // observations in place; a different plan or quote invalidates them outright.
  const dataKey = enabled && mode === 'live' && successful.length
    ? JSON.stringify([planIdentity, successful.map(quote => [quote.mint, quote.usdcRaw, quote.outRaw, quote.units, quote.fetchedAt])]) : '';
  const requestKey = dataKey ? JSON.stringify([dataKey, refresh]) : '';

  useEffect(() => {
    // Observations read for one plan and quote never stand in for another, so a new
    // plan clears the purchase gate at once. A re-read of the same plan does not: its
    // current observations stay in place until the new ones verify.
    if (published.current !== dataKey) { published.current = dataKey; onData?.(null); }
    if (!requestKey) {
      return;
    }
    ...
    void (async () => {
      ...
      try {
        ...
        if (!controller.signal.aborted) { setResult({ key: dataKey, data }); onData?.(data); }
      } catch {
        // A reading that cannot be verified is never presented, and it never leaves an
        // earlier reading standing in for it in the purchase gate.
        if (!controller.signal.aborted) { setResult({ key: dataKey, error: 'Pyth market references could not be verified. Your contribution estimates are still available.' }); onData?.(null); }
      } finally { if (!controller.signal.aborted) setReading(false); }
    })();
    return () => controller.abort();
  }, [requestKey, dataKey, mintKey, onData]);

  // An observation lives sixty seconds from its own publish time, so the panel
  // re-reads inside that window instead of sitting on a reading it has labelled
  // stale. A hidden tab reads nothing until it is looked at again, and a re-read
  // never extends the lifetime of an older quote.
  useEffect(() => {
    if (!requestKey) return;
    const startedAt = Date.now();
    const reread = () => setRefresh(value => value + 1);
    const timer = setInterval(() => { if (document.visibilityState === 'visible') reread(); }, PANEL_REFRESH_MS);
    // A tab nobody is looking at reads nothing, so it reads once on return whenever
    // its displayed observations have outlived the window.
    const resume = () => { if (document.visibilityState === 'visible' && Date.now() - startedAt >= PANEL_REFRESH_MS) reread(); };
    document.addEventListener('visibilitychange', resume);
    return () => { clearInterval(timer); document.removeEventListener('visibilitychange', resume); };
  }, [requestKey]);

  if (!requestKey) return null;
  // A re-read of the same plan keeps its current observations on screen until the new
  // ones verify; their own publish times keep labelling how old they are.
  const current = result?.key === dataKey ? result : null;
  const pending = current === null;
  const data = current?.data;
  // Only assets with an actual reading get a card: an asset the response carries no
  // observation for has nothing to compare, and the panel claims nothing for it.
  const observed = data?.items.filter(item => item.underlying || item.token) ?? [];
  // A currency observation on its own is worth showing: keyless deployments read
  // USDC/USD from chain, and that reading converts the budget.
  const hasObservations = Boolean(data?.usdc) || observed.length > 0;
  ...
  return <section className={styles.panel} aria-labelledby="market-reference-title" data-market-reference>
    <div className={styles.heading}>
      ...
      <button type="button" className={styles.refresh} disabled={pending || reading} onClick={() => setRefresh(value => value + 1)} aria-label="Refresh market references"><RefreshCw size={15} />Refresh references</button>
    </div>
    {/* The comparison is described once it is actually on screen, never before a reading verifies it. */}
    <p className={styles.intro}>{observed.length > 0 ? EQUITY_INTRO : SCOPE_INTRO}</p>
    {pending ? <p className={styles.status} role="status"><LoaderCircle size={16} className="spinning" />Checking reference freshness…</p> : !hasObservations ? <p className={styles.status} role="status">{current?.error ?? 'Your Jupiter estimates stand on their own for this selection, and no reference price is claimed for it.'}</p> : <>
