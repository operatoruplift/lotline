...
type Props = { mode: Mode; basket: Basket; assets: Asset[]; quotes: Quote[]; scheduleOccurrenceId?: string };
...
export function ExecutionReview({ mode, basket, assets, quotes, scheduleOccurrenceId }: Props) {
  ...
  useEffect(() => { latestFingerprint.current = mode === 'live' ? currentFingerprint : null; }, [currentFingerprint, mode]);
  ...
  const createReview = useCallback(async () => {
    if (!config?.enabled || !config.limits || !account || !plan.valid || !allQuotesReady || blocked || signing.current) return;
  ...
  }, [account, allQuotesReady, assetByMint, blocked, config, loadRun, plan, scheduleOccurrenceId]);
  ...
  const signAndSubmit = useCallback(async () => {
    if (!review || !selectedWallet || !account || signing.current || blocked || !hasCompleteFees(review)) return;
    ...
    try {
      await navigator.locks.request('lotline:wallet-approval', { ifAvailable: true }, async lock => {
        ...
        if (review.fingerprint !== latestFingerprint.current || Date.parse(review.expiresAt) <= Date.now()) throw new Error('This review changed or expired. Request a fresh order before signing.');
        ...
        if (review.fingerprint !== latestFingerprint.current) throw new Error('The plan or wallet changed during approval. The signed bytes were not submitted.');
  ...
  }, [account, blocked, loadRun, review, selectedWallet]);
  ...
  return <section className="execution-card" aria-labelledby="execution-heading">
    <div className="execution-card-heading"><div><div className="panel-kicker">04 <span /> EXECUTION READINESS</div><h2 id="execution-heading">Review before you sign.</h2></div><LockKeyhole size={18} /></div>
    {!config?.enabled ? <div className="execution-gate"><p>Signing is deliberately switched off in this public build. Lotline still computes the exact order for every asset — you complete the contribution yourself using the prefilled Jupiter links above.</p><details className="execution-readiness"><summary>Operator readiness detail</summary><ul>{(config?.reasons ?? ['Checking server readiness…']).map(reason => <li key={reason}>{reason}</li>)}</ul></details><span><ShieldCheck size={14} /> No wallet prompt or transaction is created while signing is switched off.</span></div> : null}
    {(config?.enabled || runId) && <>
      ...
      <div className="execution-actions">
        {(blocked || !config?.enabled) ? <button type="button" className="button secondary" onClick={() => { if (runId) void loadRun(runId, false); }} disabled={busy || restoring || !runId}>{busy || restoring ? 'Checking receipt…' : 'Check original receipt'}</button> : review ? <>
          ...
          {(!reviewMatchesPlan || expired) ? <button type="button" className="button secondary" onClick={() => setReview(null)} disabled={busy}>Update review</button> : <button type="button" className="button primary" onClick={() => void signAndSubmit()} disabled={busy || restoring || !feesReady}>{busy ? <><LoaderCircle size={15} className="spinning" />Waiting…</> : <>Sign this purchase <ArrowRight size={15} /></>}</button>}
        </> : <button type="button" className="button primary" onClick={hasMatchingRun ? resumeRemaining : createReview} disabled={busy || restoring || !account || !plan.valid || (!hasMatchingRun && !allQuotesReady) || (hasMatchingRun && (confirmedCount === runLegs.length || terminalFailure))}>{busy || restoring ? <><LoaderCircle size={15} className="spinning" />{restoring ? 'Restoring review…' : 'Preparing review…'}</> : hasMatchingRun ? <>Resume remaining <ArrowRight size={15} /></> : <>Review purchase <ArrowRight size={15} /></>}</button>}
      </div>
      {review && <div className="execution-review">
        ...
        <div><span>Token fee</span><strong>{review.feeBps === undefined || !review.feeMint ? 'Unavailable' : `${review.feeBps} basis points (${review.feeMint})`}{review.platformFee ? `; platform portion ${review.platformFee.amount} raw units` : ''}</strong></div>
