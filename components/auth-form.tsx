...
export function AuthForm({ mode, confirmationError = false }: { mode: AuthMode; confirmationError?: boolean }) {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (result.error) { setFailed(true); setMessage(errorMessage(result.error.code, result.error.status)); return; }
      form.reset();
  ...
  return <div className={styles.authGrid}>
    ...
    <section className={styles.formCard} aria-labelledby="auth-title">
      ...
      <div className={styles.switchLink}>{mode === 'sign-in' ? <>New to Lotline? <Link href="/sign-up">Create an account</Link></> : <>Already have an account? <Link href="/sign-in">Sign in</Link></>}</div>
