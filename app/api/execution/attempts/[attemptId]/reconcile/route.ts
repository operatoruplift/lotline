...
export async function POST(request: Request, context: { params: Promise<{ attemptId: string }> }) {
  try {
    ...
    const found = await getAttempt(user.id, attemptId);
    ...
    if (receipt.err !== null) {
      await transitionAttempt(user.id, attemptId, 'failed-onchain', { reason: 'Solana reported an on-chain error.', rpc: receipt.err });
    ...
    if (transaction.data.meta.err !== null) { await transitionAttempt(user.id, attemptId, 'failed-onchain', { reason: 'Solana transaction metadata reports an on-chain error.', rpc: transaction.data.meta.err }); return json({ state: 'failed-onchain', signature: found.attempt.signature, message: 'Solana reported an on-chain error. No retry was created.' }); }
    await transitionAttempt(user.id, attemptId, 'confirmed', { reason: 'Solana returned a confirmed receipt with null transaction error.', confirmationStatus: receipt.confirmationStatus, slot: transaction.data.slot });
