...
export async function POST(request: Request, context: { params: Promise<{ runId: string; legId: string }> }) {
  try {
    ...
    if (activeAttempt) {
      if ((activeAttempt.state === 'review-required' || activeAttempt.state === 'awaiting-wallet') && activeAttempt.provider_expires_at && Date.parse(activeAttempt.provider_expires_at) <= Date.now()) {
      ...
      } else if (activeAttempt.state === 'review-required' || activeAttempt.state === 'awaiting-wallet') {
        try { requireSemanticProof(activeAttempt.evidence, leg.input_raw, activeAttempt.minimum_output_raw); }
        catch {
          await transitionAttempt(owner, activeAttempt.id, 'expired-unbroadcast', { reason: 'The unsigned legacy order must be replaced with current semantic validation.' });
          expiredActiveAttempt = true;
        }
        ...
        return json({ state: 'in-progress', message: 'This leg already has an active attempt. Reconcile it before requesting another order.' }, 409);
      }
    }
