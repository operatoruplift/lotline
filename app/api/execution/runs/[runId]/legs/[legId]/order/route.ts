...
import { createAttempt, getSnapshot } from '@/lib/server/execution/repository';
...
export async function POST(request: Request, context: { params: Promise<{ runId: string; legId: string }> }) {
  try {
    ...
    if (activeAttempt) {
      if (activeAttempt.state === 'review-required' && typeof activeAttempt.evidence?.transaction === 'string') {
    ...
    if (['confirmed', 'rejected', 'failed-onchain', 'submitted', 'confirming', 'signed', 'awaiting-wallet'].includes(leg.state)) {
    ...
    const attempt = await createAttempt(user.id, runId, legId, order);
