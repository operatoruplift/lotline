...
export async function POST(request: Request, context: { params: Promise<{ runId: string; legId: string }> }) {
  try {
    ...
    const found = await getAttemptByRequestId(user.id, parsed.data.requestId);
    ...
    if (found.attempt.provider_expires_at && Date.parse(found.attempt.provider_expires_at) <= Date.now() && (found.attempt.state === 'review-required' || found.attempt.state === 'awaiting-wallet')) {
      await transitionAttempt(user.id, found.attempt.id, 'expired-unbroadcast', { reason: 'The approved provider order expired before submission.' });
    ...
    await transitionAttempt(user.id, found.attempt.id, 'awaiting-wallet', { reason: 'The user chose to sign the reviewed order.' });
    ...
    await transitionAttempt(user.id, found.attempt.id, 'signed', { reason: 'The wallet signature passed exact message and Ed25519 checks.', expectedSignature: signed.chainSignature }, signed.chainSignature);
    ...
    try {
      ...
      await transitionAttempt(user.id, found.attempt.id, 'unknown', { reason: 'The provider response was inconclusive. Reconcile this attempt before any retry.', error: error instanceof Error ? error.message : 'provider-unavailable' });
      throw error;
    }
    if (response.status === 'Failed') {
      await transitionAttempt(user.id, found.attempt.id, 'rejected', { reason: 'Jupiter rejected the signed order before confirmation.', code: response.code ?? null });
    ...
    await recordSubmitted(user.id, found.attempt.id, response.signature!, { status: response.status, inputAmountResult: response.inputAmountResult ?? null, outputAmountResult: response.outputAmountResult ?? null });
