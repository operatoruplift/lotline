...
export async function GET() {
  ...
  catch (error) { const status = error instanceof ServiceError && error.kind === 'invalid-input' ? 401 : 503; return result({ state: error instanceof ServiceError ? error.kind : 'unavailable', message: error instanceof Error ? error.message : 'Saved contribution reminders are temporarily unavailable.' }, status); }
}
...
export async function PATCH(request: Request) {
  ...
  catch (error) { return result({ state: error instanceof ServiceError ? error.kind : 'unavailable', message: error instanceof Error ? error.message : 'The contribution reminder could not be updated.' }, error instanceof ServiceError && error.kind === 'invalid-input' ? 400 : 503); }
}
