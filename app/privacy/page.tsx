...
export default function PrivacyPage() {
  return <><SiteHeader /><main id="main" className="privacy-page page-width">
    ...
    <section><h2>When you request Live data</h2><p>The server sends a public wallet address to the configured Solana RPC provider only when you load balances. It requests issuer metadata from xStocks or PreStocks and quote estimates from Jupiter. Quote requests contain token mints and the allocated USDC amount, with no wallet address or taker.</p><p>After successful xStocks estimates, the optional Pyth reference service requests the mapped public feed IDs. It receives no wallet address, budget, account identity, or contribution percentages. Missing Pyth access does not block planning.</p><p>Providers and the hosting platform may retain operational and security logs under their own policies. Lotline does not add advertising trackers or analytics. Short-lived server caches help avoid repeated provider requests.</p></section>
