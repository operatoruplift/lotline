...
const mintsSchema = z.array(addressSchema).min(1).max(3).refine(items => new Set(items).size === items.length);
...
export const quotesRequestSchema = z.object({ items: z.array(z.object({ mint: addressSchema, usdcRaw: z.string().refine(value => isBoundedRaw(value, MAX_USDC_RAW)) }).strict()).min(1).max(3) }).strict().refine(value => new Set(value.items.map(item => item.mint)).size === value.items.length).refine(value => value.items.every(item => isBoundedRaw(item.usdcRaw, MAX_USDC_RAW)) && value.items.reduce((sum, item) => sum + BigInt(item.usdcRaw), 0n) <= MAX_USDC_RAW);
export const unitsRequestSchema = z.object({ items: z.array(z.object({ mint: addressSchema, raw: rawSchema }).strict()).min(1).max(3) }).strict().refine(value => new Set(value.items.map(item => item.mint)).size === value.items.length);
