...
export const scheduleSchema = z.object({ id: z.string().uuid().optional(), name: z.string().trim().min(1).max(60), budgetRaw: raw.refine(value => isBoundedRaw(value, MAX_USDC_RAW) && BigInt(value) > 0n), allocations: scheduleAllocations, cadence: z.enum(['weekly', 'monthly']), timezone: timezoneSchema, nextDueAt: z.string().datetime({ offset: true }), paused: z.boolean().default(false) }).strict();
