...
export async function createRun(owner: ExecutionOwner, intent: ContributionIntent, intentHash: string): Promise<ExecutionSnapshot> {
  ...
  if (owner.kind === 'user' && owner.guestCapabilityHash) {
    ...
    if (guestExisting) return hydrateSnapshot(db, guestExisting);
  }
...
export async function createAttempt(owner: ExecutionOwner, runId: string, legId: string, order: ExecutionOrder): Promise<AttemptRow> {
  ...
  check(await db.from('lotline_execution_runs').update({ state: 'quoting', updated_at: now }).eq('id', runId));
  ...
  check(await db.from('lotline_execution_runs').update({ state: 'review-required', updated_at: new Date().toISOString() }).eq('id', runId));
...
export async function transitionAttempt(owner: ExecutionOwner, attemptId: string, state: ExecutionState, evidence: Record<string, unknown> = {}, signature?: string): Promise<void> {
  ...
  const db = client();
  const now = new Date().toISOString();
  const changed = await db.from('lotline_execution_attempts').update({ state, ...(nextSignature ? { signature: nextSignature } : {}), evidence, state_version: found.attempt.state_version + 1, updated_at: now }).eq('id', attemptId).eq('leg_id', found.leg.id).eq('state', found.attempt.state).eq('state_version', found.attempt.state_version).select('id').maybeSingle();
  ...
  check(await db.from('lotline_execution_runs').update({ state, updated_at: now }).eq('id', found.run.id));
