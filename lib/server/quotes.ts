...
export function normalizeQuote(payload: unknown, mint: string, usdcRaw: string, fetchedAt = new Date().toISOString()): Quote {
  ...
  if (!parsed.success || parsed.data.inputMint !== USDC_MINT || parsed.data.outputMint !== mint || parsed.data.inAmount !== usdcRaw) throw new ServiceError('unavailable', 'No verified quote is available for this amount. Try a different amount or refresh.', 'provider-unavailable');
  if (BigInt(parsed.data.outAmount) <= 0n) throw new ServiceError('unavailable', 'No route returns output for this amount. Try a different amount or refresh.', 'no-route');
  const data = parsed.data;
  if (data.errorCode !== undefined || data.errorMessage) throw new ServiceError('unavailable', 'Jupiter could not provide a usable route. Try again later.', 'provider-unavailable');
  const expiry = Math.min(Date.parse(fetchedAt) + 30_000, data.expireAt ? Date.parse(data.expireAt) : Infinity);
  if (expiry <= Date.now()) throw new ServiceError('unavailable', 'The provider quote already expired. Refresh estimates.', 'stale-verification');
...
export function unavailableQuote(mint: string, usdcRaw: string, message: string, reasonCode: DataFailureReason = 'provider-unavailable'): Quote {
...
async function getQuote(asset: Asset, usdcRaw: string): Promise<Quote> {
  ...
  if (!promise) {
    promise = freshQuote(asset, usdcRaw).then(quote => {
      ...
      return quote;
    }).catch(error => unavailableQuote(asset.mint, usdcRaw, safeMessage(error), error instanceof ServiceError ? error.reasonCode : undefined)).finally(() => pending.delete(key));
