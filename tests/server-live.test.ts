...
it.skipIf(process.env.LOTLINE_LIVE_SMOKE !== '1')('real issuer, chain holdings, mint scaling and amount-specific quote smoke', async () => {
  ...
  // Keep fresh observations outside the dated, committed historical evidence.
  await mkdir(new URL('../test-results/', import.meta.url), { recursive: true });
  await writeFile(new URL('../test-results/live-smoke.json', import.meta.url), `${JSON.stringify(evidence, null, 2)}\n`);
  expect(catalog.assets).toHaveLength(XSTOCK_REGISTRY.length);
