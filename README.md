...
**September 20 release:** contribution recovery, downloadable receipts, manual reminder sync and a clearly labeled controlled demonstration are deployed in application commit `bae159d`. All six Supabase migrations are applied, and production flags keep purchases disabled while allowing configured receipt reconciliation. CI passed 336 deterministic tests and 80 browser scenarios; a separate hosted read-only check verified the 832-identity catalog, public holdings, Jupiter estimates and scaled units. The selected L1/L4/L5/L6 design, brand kit and narrated planner films are preserved. See the [current deployment record](docs/deployment.md) and [September 20 release evidence](docs/release-20260920.md). In-app mainnet purchases still require the missing encoded swap validator, Jupiter credentials and issuer access policy; a test recording does not prove a real purchase. No competition submission or funded transaction has been made.
