...
| Loader | Loader Animation, listed in the [official catalog](https://motionsites.ai/?prompt=guardnet-landing) | Use Lotline's selected branching mark with a gentle pulse and a text status. Animate only while a request is active. No artificial startup delay, looping marketing intro, or percentage that does not represent measured progress. |
...
Use warm off-white `#F7F7F2`, white panels, forest `#174D3C`, charcoal `#18211D`, and soft gray-green borders. Amounts use tabular numerals. Keep body text legible, labels outside fields, control targets comfortable on touch screens, and statuses understandable without color. The user selected the original branching symbol from revised concept 02 on September 12, 2026. Use its shared SVG geometry throughout the application; see [brand sources and exports](brand.md).
