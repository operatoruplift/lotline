...
This document describes current source behavior. Hosted observations below retain their original dates and origins; the current redesign and finish work have not been deployed by this task. Public signup and recovery remain gated, while existing-account sign-in is available when Supabase is configured.
