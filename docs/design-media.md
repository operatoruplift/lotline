...
The UI owns decorative playback: muted, inline, explicit visible motion controls, reduced-motion preference changes, rejection-safe autoplay, poster fallbacks, visibility-aware loading, pause offscreen/in background and cleanup on route changes. The build script deliberately does not alter application logic or make playback a prerequisite for any planner or authentication action. Glass refraction must sample this real same-origin video behind crisp native form controls; CSS blur is only its readable fallback.
